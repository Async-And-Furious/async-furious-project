...
describe('PecaRepository', () => {
  ...
  beforeEach(async () => {
    ...
    repository = module.get<PecaRepository>(PecaRepository);
  });
  ...
  describe('create', () => {
    it('should create a peca with all fields', async () => {
      ...
      mockPrismaService.peca.create.mockResolvedValue({ ...createData, id: '123', created_at: new Date(), updated_at: new Date() } as never);
  ...
  describe('findAll', () => {
    it('should return pecas with pagination', async () => {
      mockPrismaService.peca.findMany.mockResolvedValue([mockPeca] as never);
  ...
  describe('findOne', () => {
    it('should return a peca by id', async () => {
      mockPrismaService.peca.findUnique.mockResolvedValue(mockPeca as never);
    ...
    it('should throw NotFoundException when peca not found', async () => {
  ...
  describe('update', () => {
    it('should update a peca', async () => {
      ...
      mockPrismaService.peca.findUnique.mockResolvedValue(mockPeca as never);
      mockPrismaService.peca.update.mockResolvedValue({ ...mockPeca, nome: 'Filtro de Ar' } as never);

      const result = await repository.update('123', updateData);

      expect(result.nome).toBe('Filtro de Ar');
      expect(mockPrismaService.peca.update).toHaveBeenCalledWith({ where: { id: '123' }, data: updateData });
    });

    it('should throw NotFoundException when peca not found', async () => {
      mockPrismaService.peca.findUnique.mockResolvedValue(null);

      await expect(repository.update('nonexistent', { nome: 'X' })).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('updateEstoque', () => {
    it('should update stock quantity', async () => {
      mockPrismaService.peca.findUnique.mockResolvedValue(mockPeca as never);
      mockPrismaService.peca.update.mockResolvedValue({ ...mockPeca, quantidade_estoque: 25 } as never);
  ...
  describe('remove', () => {
    it('should delete a peca', async () => {
      mockPrismaService.peca.findUnique.mockResolvedValue(mockPeca as never);
      mockPrismaService.peca.delete.mockResolvedValue(mockPeca as never);
    ...
    it('should throw NotFoundException when trying to delete nonexistent peca', async () => {
      mockPrismaService.peca.findUnique.mockResolvedValue(null);
