...
@Injectable()
export class OrdemDeServicoRepository implements IOrdemDeServicoRepository {
  ...
  async remove(id: string): Promise<OrdemDeServico> {
    await this.findOne(id);
    return (await this.prisma.serviceOrder.delete({ where: { id } })) as unknown as OrdemDeServico;
  }
}
